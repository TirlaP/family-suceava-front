...
interface InstructorSlugPageProps {
  params: {
    instructorSlug: string;
  };
}

export default function InstructorSlugPage({ params }: InstructorSlugPageProps) {
  const [instructor, setInstructor] = useState<Instructor | null>(null);
  const [instructorClasses, setInstructorClasses] = useState<DanceClass[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const { instructorSlug } = params;
    ...
    const loadInstructorAndClasses = async () => {
      try {
        ...
        const foundInstructor = instructors.find(
          (inst) => inst.attributes.name.toLowerCase().replace(/\s+/g, '-') === instructorSlug
        );
        ...
        <div className="absolute inset-0 flex items-center">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
            ...
            >
              ...
              <p className="text-xl text-gray-200 mb-6">{specialties?.join(" • ")}</p>
